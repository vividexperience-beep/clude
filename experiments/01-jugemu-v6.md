...
### 第1ラウンド(改行11行・ルビなし・笑い声要求あり)

- **読みが間違う**
- **観客の笑い声が無関係な箇所にも入り、邪魔になる**(予告したリスクがそのまま出た)
- **言い立てが区切られる**(11行に割っていたので当然)

### 第2ラウンド(かな1行・ルビ付き・笑い声除外)

**ユーザー評価: 「語り口調はかなり落語家だった、あとは名前のくだりはやはり歌いたいっぽいw」**

- **語り口調は成立した。** v5.5世代の「現状は微妙」から**明確に前進している**。
  口上・マクラ・会話部分は落語として通用するレベルに達した。
- **言い立てだけが依然として歌になる。** 反復＝歌のバイアスはv6でも健在。

### 結論(中間)

**v6で「語り」は解決した。残る壁は「反復」だけ。**

これは外部ソースにも明記があった:
> **Rhyme, repeated hooks, chorus labels and melodic Style terms all encourage singing.**
> (韻・**反復されたフック**・コーラスラベル・旋律的なスタイル語は、いずれも歌唱を誘発する)

**つまりバグではなく仕様。** タグで打ち消しにいくのは筋が悪い
(`songs/15-nemuri-mizu.md`で何重にかけても勝てなかったのと同じ相手)。

**未試行の対策(次にやるならここから)**
1. **ハイフンで音節を分離** — 同ソースが
   「カンマやハイフンは各セグメントを別々の音として扱わせる(`P-p-please`は3チャンク)」と報告。
   **改行(=息継ぎ)とは別物なので、「一息で」と両立する。**
   ただし**日本語のかな文字列で同じ挙動になる保証はない。**
2. **言い立ての直前に`[Spoken Word]`を再投入** — 現状は冒頭1回のみ。
   長い生成の途中で効力が薄れている可能性
3. **歌えない速度を要求する** — 歌は音を伸ばす余地が要るので、
   早口で詰めれば物理的に歌えない。`Rapid Fire Tongue Twister, Too Fast To Sing`等。**完全に未検証**
